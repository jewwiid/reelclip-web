...
export default async function CompetitorsPage() {
  ...
  return <><Nav /><main>
    ...
    <section className="px-6 py-20 sm:px-10 sm:py-28"><div className="mx-auto max-w-3xl text-center"><h2 className="mb-4 text-3xl font-black tracking-tight sm:text-4xl">{copy.ctaHeading}</h2><p className="mx-auto mb-8 max-w-xl text-base leading-relaxed text-text-muted">{copy.ctaBody}</p><div className="flex flex-col items-center justify-center gap-3 sm:flex-row"><a href="https://apps.apple.com/app/reelclip/id6787742864" target="_blank" rel="noreferrer" data-event-name="download_click_compare" className="accent-glow w-full rounded-full bg-accent px-7 py-3.5 text-base font-bold text-bg transition hover:bg-accent-deep sm:w-auto">{copy.download}</a><Link href={localizedPath(locale, "/pricing")} className="w-full rounded-full border border-hairline px-7 py-3.5 text-base font-semibold text-text transition hover:bg-surface sm:w-auto">{copy.seePricing}</Link></div></div></section>
  </main><Footer /></>;
}
