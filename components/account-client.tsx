...
export function AccountClient() {
  ...
  async function lookup() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? copy.lookupFailed.replace("{status}", String(res.status)));
  ...
  async function openPortal() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.ok) {
        throw new Error(data.error ?? copy.portalFailed.replace("{status}", String(res.status)));
      }
  ...
  const tierLabel = (t: string) =>
    t === "creator" || t === "studio" ? copy.creator : copy.free;
  ...
  const formatDate = (ms: number) => {
    if (!ms || !Number.isFinite(ms)) return copy.unknownDate;
    try {
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-2xl border border-hairline bg-control-surface p-6 sm:p-8">
        <h2 className="text-lg font-bold tracking-tight">{copy.lookupHeading}</h2>
        ...
        <div className="mt-5 grid sm:grid-cols-2 gap-4">
          ...
          <label className="block">
            <span className="block text-xs uppercase tracking-wide text-text-muted mb-1.5">
              {copy.token} <span className="text-text-muted/60">({copy.optional})</span>
            </span>
            <input
              type="text"
        ...
        <div className="rounded-2xl border border-hairline bg-control-surface p-6 sm:p-8">
          ...
          <div className="mt-6">
            <p className="text-xs uppercase tracking-wider text-text-muted font-bold mb-3">
              {copy.activeEntitlements.replace("{count}", String(result.entitlements.length))}
            </p>
            {result.entitlements.length === 0 ? (
              <p className="text-sm text-text-muted">
                {copy.noEntitlements} <a href={localizedPath(locale, "/pricing")} className="underline hover:text-text">{copy.viewPricing}</a> {copy.upgradeSuffix}
              </p>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-text-muted mt-0.5">
                        ...
                        {e.cancelAtPeriodEnd ? ` · ${copy.cancelsAtEnd}` : ""}
                      </p>
                    </div>
                    <p className="text-xs text-text-muted whitespace-nowrap">
                      {copy.renews} {formatDate(e.currentPeriodEnd)}
                    </p>
                  </li>
                ))}
              </ul>
