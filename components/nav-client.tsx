  ...
  return (
    <nav className="relative z-20 border-b border-hairline bg-bg/95 backdrop-blur-sm">
      <div className="flex items-center justify-between gap-4 px-5 py-4 sm:px-8 lg:px-10 lg:py-5">
        <Link href={locale === "en" ? "/" : `/${locale}`} onClick={closeMenu} className="flex shrink-0 items-center group">
          <Image
            src="/wordmark.png"
            alt="ReelClip"
            width={140}
        ...
        <div className="hidden items-center gap-4 text-[13px] text-text-muted lg:flex xl:gap-6 xl:text-sm">
          ...
          <LanguageSwitcher locale={locale} label={language.label} selectLabel={language.select} />
          <a
            href={APP_STORE_URL}
            target="_blank"
            rel="noreferrer"
        ...
        <div className="flex items-center gap-2 lg:hidden">
          ...
          >
            ...
            <span aria-hidden="true" className={`absolute h-0.5 w-4 rounded-full bg-current transition-opacity ${isMenuOpen ? "opacity-0" : "opacity-100"}`} />
            <span aria-hidden="true" className={`absolute h-0.5 w-4 rounded-full bg-current transition-transform ${isMenuOpen ? "-rotate-45" : "translate-y-1.5"}`} />
          </button>
        </div>
      </div>
      ...
      {isMenuOpen && (
        <div id="mobile-navigation" className="absolute inset-x-0 top-full border-b border-hairline bg-bg/98 px-5 py-5 shadow-2xl backdrop-blur-md lg:hidden">
